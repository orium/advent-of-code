...
fn highest_peak(initial_speed: Vector, target_area: &Area) -> Option<i64> {
    let mut peak = 0;
    ...
    for t in 0.. {
        ...
        if !hit_area && target_area.contains(pos) {
            hit_area = true;
        }
    }
    ...
    */
    ...
    for y_speed in *target_area.y_range.start() .. {
        for x_speed in 1 ..= *target_area.x_range.end() {
            ...
            if let Some(h) = highest_peak(initial_speed, &target_area) {
                if h > peak {
                    peak = h;
                    println!("peak: {}", peak);
                }
