...
fn get_bin_val(bin: &[u8], index: usize, length: usize) -> u32 {
    let mut r = 0;

    for i in 0..length {
        r = (r << 1) | (bin[index + i] as u32);
    }
        ...
        _ => {
            ...
            match length_type {
                0 => {
                    ...
                    (start + consumed, Packet { version, expr: Expr::Op(operands) })
                },
                1 => {
...
fn add_version_nums(packet: &Packet) -> u32 {
    packet.version + match packet.expr {
        Expr::Num(_) => 0,
        Expr::Op(ref packets) => packets.iter().map(add_version_nums).sum(),
    }
}
...
fn main() -> io::Result<()> {
    ...
    for line in reader.lines() {
        ...
        // println!("{:#?}", packet);
        println!("{}", add_version_nums(&packet));
    }
